import { useDeleteRewardConditionMutation } from "@/app/redux/apis/rewardsApiQ"
...
import { Comparator, Reward, RewardCondition } from "@/models/reward"
import { zodResolver } from "@hookform/resolvers/zod"
import { Trash2 } from "lucide-react"
import { UseFormReturn, useFieldArray, useForm } from "react-hook-form"
import { addConditionValidator } from "../validators"

export interface RewardConditionEditorProps {
    formManager: UseFormReturn<Partial<Reward>>
    isUpdate?: boolean
}

export default function RewardConditionEditor(props: RewardConditionEditorProps) {
    const conditionForm = useForm<RewardCondition>({
    ...
    const [rewardDelete] = useDeleteRewardConditionMutation()
    ...
    const onConditionAdd = (value: RewardCondition) => {
        conditionFieldManager.append(value)
    }

    const deleteCondition = async (arrIdx: number) => {
        if (props.isUpdate) {
            const condition = conditionFieldManager.fields[arrIdx] as unknown as RewardCondition

            if (condition) {
                try {
                    await rewardDelete(condition)
                } catch (error) {
                    return
                }
            }
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {conditionFieldManager.fields.map((c, idx) => (
                                    <TableRow key={`${c.field}-${c.comparator}-${c.value}-${idx}`}>
                                        <TableCell className="text-center">{idx + 1}</TableCell>
                                        <TableCell className="text-center">{c.field}</TableCell>
                                        <TableCell className="text-center">{c.comparator}</TableCell>
